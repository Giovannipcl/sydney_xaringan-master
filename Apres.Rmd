...
- We present an alternative Bayesian estimation procedure based on the Laplace approximation, motivated by the Integrated Nested Laplace Approximation (INLA) approach.
...
- These new transformations from BCTM, $h(Y|\boldsymbol{x}): \mathbb{R} \rightarrow \mathbb{R}$, are functions of univariate response variable conditional on fixed values of explanatory variables $\boldsymbol{X} = \boldsymbol{x}$.
- The BCTM class is composed of a transformation function $h(Y|\boldsymbol{x})$ and a pre-defined cumulative distribution function $F_Z$
...
- $F_Z$ is the continuous baseline distribution.
- A reasonable choice for $F_Z$ is the standard Normal distribution. 
- The $h(Y|\boldsymbol{x})$ function is seen as standardization since it transforms the response variable conditionally on $\boldsymbol{x}$ such that it follows a pre-defined conditional distribution function $F_Z: \mathbb{R} \rightarrow [0,1]$. 
- The BCTM's transformation functions  $h(Y|\boldsymbol{x})$ range from low-parameterized and interpretable models to complex structures with many parameters. 
...
- The basis splines functions includes good approximation to most smooth functions \citep{green1993nonparametric}
- There is several spline basis. Here we used B-splines (ref deboor).
- The B-spline allow to control the smoothness of the function. 
- The B-spline has a relevant application called P-splines ().
...
- For degree $k = 2$ with a vector of inner knots $(2,3, \dots, 8)$ of length 7 we should have a vector of knots $\boldsymbol{t} = (t_1, t_2, 2,3, \dots, 8, t_9, t_10)$  The total of curve and linear parameters is $q = 11 - 2\times 2 = 8$.
- j = 1
...
- We consider an alternative parameterization of the spline parameters to produce monotonically increasing spline function (REF). 
- Let $\boldsymbol{\gamma} = (\gamma_1, \hdots, \gamma_q)$ in terms of unconstrained parameters $\boldsymbol{\beta}$, that is represented by

$$\begin{equation}\label{beta_mono}
  \gamma_1 = \beta_1, \hspace{0.2cm} \gamma_l = \beta_1 + \sum_{i = 2}^l \exp(\beta_i), i = 2, \hdots, l.  
\end{equation}$$
---
...
\noindent is an $l \times l$ matrix, and 

$$\begin{equation}
    \boldsymbol{\tilde{\beta}} = (\beta_1, \exp(\beta_2), \hdots, \exp(\beta_l))^T.
\end{equation}$$
...
- Now, the restriction of monotonicity is necessary only in the $y$ direction. In the matrix notation

$$\begin{equation}
    \Sigma = \Sigma_1 \otimes I_2,
\end{equation}$$
-  the vector of the parameters is given by

$$\begin{equation}\label{betatilde}
    \boldsymbol{\tilde{\beta}} = (\beta_{j11}, \beta_{j12}, \hdots, \beta_{j1I_2}, \exp(\beta_{j21}), \exp(\beta_{j22}), \hdots, \exp{\beta_{j2I_2}}, \hdots, \exp(\beta_{jI_1,I_2}))^{\top} 
\end{equation}$$
- From this construction, we have $L_1 \times L_2$ parameters to consider for a particular transformation function $h(y|x)$.
---
...
- This construction faces some problems, like the number of knots.
- A popular and well-documented technique to deal with this situation is based on the fitting penalty.
- . In the B-spline context, an appealing penalty based on the finite difference of adjacent basis coefficient is called P-Splines `r Citep(myBib,"eilers1996flexible")`. This penalized approach aims to estimate a sufficient flexible curve considering a moderate number of equally spaced knots.
- In the Bayesian context, smoothness and regularization are accomplished through adequate prior distributions for model parameters. In the Bayesian P-Splines approach `r Citep(myBib,"lang2004bayesian")`, we set multivariate normal distributions with zero vector mean and precision matrix $\boldsymbol{K}$.
---
...
$$\begin{equation}\label{spline_f}
    h_j(y) = \sum_{l_1 = 1}^{L_1} B_{j,l_1,3,\boldsymbol{l}_1}(y) \gamma_{l_1} =  \boldsymbol{B}_j \boldsymbol{\gamma}^T =  \boldsymbol{B}_j \Sigma \boldsymbol{\Tilde{\beta}}  =  \boldsymbol{X}_j \boldsymbol{\Tilde{\beta}}.
\end{equation}$$


.content-box-gray[- Sum-to-zero restriction
    $$\boldsymbol{1}^T\boldsymbol{X}_j \boldsymbol{\Tilde{\beta}} = 0, \forall \tilde{\boldsymbol{\beta}}$$
    $$\rightarrow \boldsymbol{1}^T\boldsymbol{X}_j = \boldsymbol{0}$$
]

---

- A method to achieve this restriction is using QR decomposition of $(\boldsymbol{1}^T\boldsymbol{X}_j)^T$. Set as $\boldsymbol{Z}$ the final $L_1 - 1$ columns of the orthogonal matrix from the QR decompostion of $(\boldsymbol{1}^T\boldsymbol{X}_j)^T$, then we rewrite $\Tilde{\boldsymbol{\beta}}$ as 

$$\begin{equation}
     \Tilde{\boldsymbol{\beta}} = \boldsymbol{Z} \Tilde{\boldsymbol{\beta}}'
 \end{equation}$$
 
- This construction will satisfy the constraint $\boldsymbol{1}^T\boldsymbol{X}_j \boldsymbol{\Tilde{\beta}} = 0$ for any value of the $L_1 - 1$ dimensional vector $\Tilde{\boldsymbol{\beta}}'$. 
- In practice, we post multiply $\boldsymbol{X}_j$ by $\boldsymbol{Z}$ and set the first element of $\Tilde{\boldsymbol{\beta}}$ to zero. 
- The prior precision matrix for $\Tilde{\boldsymbol{\beta}}'$ is the $\boldsymbol{K}_1$ with the first row and column removed.
...
$$\boldsymbol{\tilde{\beta}} = (\beta_{j11}, \beta_{j12}, \hdots, \beta_{j1L_2}, \exp(\beta_{j21}), \exp(\beta_{j22}), \hdots, \exp{\beta_{j2L_2}}, \hdots, \exp(\beta_{jL_1,L_2}))^{\top}$$ 
-  We achieve (\ref{sumtozero}) by introducing a re-parametrization matrix $\boldsymbol{Z}$, such that $\Tilde{\boldsymbol{\beta}}_j = \boldsymbol{Z}\Tilde{\boldsymbol{\beta}}_j' $, where
...
.content-box-gray[ **Definition** The quadruple ($\vartheta$, $F_Z$, $\boldsymbol{c}$, $\pi_{\vartheta}(\dot)$)  of unknown model parameters $\vartheta$, a choice for the basis $\boldsymbol{c}$, the reference distribution $F_Z$ and joint prior $\pi_{\varphi}$ is called Bayesian Conditional Transformation Model (BCTM).
]

\noindent \textbf{Remark:} We have defined the BCTM for continuous response $Y \in \mathbb{R}$. In case of a count response, $Y \in \lbrace 0, 1, \hdots \rbrace$, the B-spline basis is applied on the log scale of response variable $Y$, that is, $h(y) = \boldsymbol{a}(log(y+1))$ \citep{carlan2022bayesian}. The construction of a monotonically B-spline is the same as the continuous case, including the prior specification for model parameters. A count model which accommodates $y$ and $x$ non-linear interaction is achieved by the continuous construction in (\ref{bctm_model}), however, applying the B-spline function on the log-scale of $y$. The $Y|\boldsymbol{X} = \boldsymbol{x}$ conditional density is expressed as 
...
\noindent where $F_Z$ is the cumulative function of standard Normal distribution. In the count response case it is no necessary to evaluate the first derivative of B-spline.
...
\end{equation}$$
where $\pi(\boldsymbol{\gamma}_s|\mu, \Sigma_s, \boldsymbol{c}_s'(\boldsymbol{y}|\boldsymbol{x}), 0)$ is the density of truncated normal multivariate with linear constraint $\boldsymbol{c}_s'(y|x)^T \geq \boldsymbol{\gamma}_s$. 
- We are interested in the marginal posterior distributions $\pi(\gamma_i|\boldsymbol{y})$
- The Laplace's method can be used to approximate these marginal densities (COLOCAR REFERENCIA).
---
...
- The final expression for marginal distributions of the parameters is already presented by \cite{tierney1986accurate}

$$\begin{equation}
    \Tilde{\pi}(\gamma_i|\boldsymbol{y}) = \left(\frac{|H_{\pi}(\boldsymbol{\gamma}(\gamma_i))|}{2\pi n |H_{\pi}(\boldsymbol{\gamma}_0)|}\right)^{1/2} \frac{\pi(\boldsymbol{\gamma}_0(\gamma_i))e^{g(y;\boldsymbol{\gamma_0}(\gamma_i)})}{\pi(\boldsymbol{\gamma}_0) e^{g(y; \boldsymbol{\gamma_0})}}. 
\end{equation}$$
...
$$\begin{equation}\label{hyper_dist}
   \Tilde{\pi}(\boldsymbol{\tau}|\boldsymbol{y}) \propto  \frac{\pi(\boldsymbol{\beta}, \boldsymbol{\tau}, \boldsymbol{y})}{\Tilde{\pi}(\boldsymbol{\beta}| \boldsymbol{\tau}, \boldsymbol{y})}\bigg|_{\boldsymbol{\beta} = \boldsymbol{\beta}^{*}(\boldsymbol{\tau})}
\end{equation}$$

where $\pi(\boldsymbol{\beta}, \boldsymbol{\tau}, \boldsymbol{y}) = \pi(\boldsymbol{y}|\boldsymbol{\tau}, \boldsymbol{\beta}) \pi(\boldsymbol{\beta}|\boldsymbol{\tau})p(\boldsymbol{\tau})$.
...
---
- The integral in the denominator of \label{condi_beta} can be approximated by Laplace  method
...
$$\begin{equation}
    \log( \Tilde{\pi}(\boldsymbol{\beta}^{*}(\boldsymbol{\tau})|\boldsymbol{y}, \boldsymbol{\tau}) \approx -\frac{n}{2} \log(2\pi) + \frac{1}{2} \log \left| -H_p(\boldsymbol{\beta}^{*}(\boldsymbol{\tau})) \right |
\end{equation}$$
-Finally, the approximated distribution in \ref{hyperdist} is given by
$$\begin{equation}\label{hyper_dist}
    \log \Tilde{\pi}(\boldsymbol{\tau}|\boldsymbol{y}) \propto \left[ \log(\pi(\boldsymbol{y},\boldsymbol{\beta}^{*}(\boldsymbol{\tau}), \boldsymbol{\tau}) - \frac{1}{2} \log \left| -H_p(\boldsymbol{\beta}^{*}(\boldsymbol{\tau})) \right| \right]_{\boldsymbol{\beta} = \boldsymbol{\beta}^{*}(\boldsymbol{\tau})}
\end{equation}$$


---
- It is necessary to explore the $\boldsymbol{\tau}|y$ distribution for the further numerical integration of marginal posterior $\beta_i|\boldsymbol{\tau}, \boldsymbol{y}$. 
.content-box-gray[
- Steps implemented to obtain density points from $\tilde{\pi}(\boldsymbol{\tau}|\boldsymbol{y})$.]
...
Colocar os gifs
---
- The next step is to approximate the posterior marginals for the $\beta_i$'s, conditioned on the explored values of $\bfs{\tau}$ from the $\Tilde{\pi}(\bfs{\tau}|\bfs{y})$ distribution. The approximated posterior marginal is

$$\begin{equation}\label{beta_marginais}
    \tilde{\pi}(\bfs{\beta}_i| \bfs{\tau}, \bfs{y}) \propto \frac{\pi(\bfs{\beta}, \bfs{\tau}, \bfs{y})}{\Tilde{\pi}(\beta_{-i} |\beta_i, \bfs{\tau}, \bfs{y})} \bigg|_{\bfs{\beta}_{-i} = \bfs{\beta}_{-i}^{*}(\beta_i, \bfs{\tau})},
\end{equation}$$
where $\bfs{\beta}_{-i}^{*}(\beta_i, \bfs{\tau})$ maximizes $\pi(\bfs{\beta}|\bfs{\tau}, \bfs{y})$ given the constraint $\beta_i^* = \beta_i$.
- The denominator in \ref{beta_marginais} is a approximation from 

$$\begin{equation}\label{beta_marginais2}
    \pi(\bfs{\beta}_{-i}|{\beta}_{i}, \bfs{\tau},\bfs{y}) = \frac{\pi(\bfs{\beta}, \bfs{\tau}, \bfs{y})}{\int \pi(\bfs{\beta}, \bfs{\tau}, \bfs{y})d\bfs{\beta}_{-i}}
\end{equation}$$
...
---
- A computational cheaper option is to base this approximation on a conditional density implied by the Gaussian approximation, $\Tilde{\pi}_G(\bfs{\beta}| \bfs{\tau}, \bfs{y})$, for $\bfs{\beta}| \bfs{\tau}, \bfs{y}$ distribution.
- The mode $\bfs{\beta}_{-i}$ now would be the conditional mean of a multivariate normal distribution given by
$$\begin{equation}\label{mode_condi}
    \bfs{\beta}_{-i}^{*}(\beta_i, \bfs{\tau}) = E_{\Tilde{\pi}_G(\bfs{\beta}_{-i} |\beta_i)} = \bfs{\beta}(\bfs{\tau})_{-i} + (H_{-i,i})^{-1}H_{i,i}\left[\beta_i -  \bfs{\beta}(\bfs{\tau})_{i}\right].
\end{equation}$$
where $\bfs{\beta}(\bfs{\tau})$ is the mode vector of $p(\bfs{\beta})$ distribution, and $\bfs{\beta}(\bfs{\tau})_{-i}$ is the vector without the ith component. The conditional Hessian is given by

$$\begin{equation}
    H_{\Tilde{\pi}_G(\bfs{\beta}_{-i} |\beta_i)} = H_{-i,-i} - (H_{-i,i})^{-1}H_{i,i}(H_{i,-i}),
\end{equation}$$
where $H$ is the Hessian from $p(\boldsymbol{\beta})$ applied to $\bfs{\beta}(\bfs{\tau})$, it is constant since is independent of $\beta_i$.


---

- The benefits of this approximation are immediate since we do not evaluate the integral in (\ref{beta_marginais2}) for each $i$ but only to find the mode of $p(\bfs{\beta})$ (already computed in the hyper-parameter posterior density exploration step) and by Normal distribution properties find the conditional expected value.
- Since the Hessian matrix is independent of $\beta_i$ then $\pi(\beta_i| \bfs{\tau}, \bfs{y}) \propto \pi(\Tilde{\bfs{\beta}}, \bfs{y}, \bfs{\tau})$. 
The last step is to integrate $\bfs{\tau}$ from $\pi(\beta_i| \bfs{\tau}, \bfs{y})$ numerically as 

$$\begin{equation}
  \int \pi(\beta_i| \bfs{\tau}, \bfs{y}) d\bfs{\tau} = \sum_{j = 1}^{J} \pi(\beta_i| \bfs{\tau}^{(j)}, \bfs{y}) \Delta_j.   
\end{equation}$$
...
-  The resulting MCMC samples are employed to estimate the conditional cumulative distribution $F_{Y|\bfs{X} = \bfs{x}}(y) = \hat{F}_{Y|\bfs{X} = \bfs{x}}(y) = F_Z(\hat{h}(y|\bfs{x}))$ where $\hat{h}(y|\bfs{x})$ is the posterior mean estimate

$$\begin{equation}
     \hat{h}(y|\bfs{x}) = \sum_{s = 1}^S \frac{1}{S}(\boldsymbol{a}_j(y)^{\top} \otimes \boldsymbol{b}_j(x)^{\top})^{\top} \boldsymbol{\gamma}_j^{(s)},
 \end{equation}$$
---

The posterior mean estimate for $F_{Y|\bfs{X} = \bfs{x}}(y)$ is 


$$\begin{equation}
    \hat{F}_{Y|\bfs{X} = \bfs{x}}(y) = \frac{1}{S} \sum_{s = 1}^S F_Z((\boldsymbol{a}_j(y)^{\top} \otimes \boldsymbol{b}_j(x)^{\top})^{\top} \boldsymbol{\gamma}_j^{(s)}).
\end{equation}$$
---
...
- The first criteria is the Deviance Information Criteria (DIC) proposed by \cite{spiegelhalter2002bayesian}. 
- The DIC is defined in terms of the deviance, $D(\bfs{\beta}, \bfs{\tau}) =-2\log p(y|\boldsymbol{\tau}, \boldsymbol{\beta})$ and a model complexity measure named as the effective number of parameters, $p_{DIC}$.

$$\begin{equation}
    \textrm{DIC} = -2 \log \pi(y| \hat{\bfs{\beta}}, \hat{\bfs{\tau}}) + 2p_{DIC},
\end{equation}$$

where, 
$$\begin{equation}\label{pdic}
    p_{DIC} = E_{\bfs{\beta}, \bfs{\tau}|\bfs{y}}[D(\bfs{\beta}, \bfs{\tau})] - D(\hat{\bfs{\beta}}, \hat{\bfs{\tau}}),
\end{equation}$$
---
...
$$\begin{equation}\label{pdic_int}
    E_{\bfs{\beta}, \bfs{\tau}|\bfs{y}}[-2\log p(y|\boldsymbol{\tau}, \boldsymbol{\beta})] = \int \int -2\log p(y|\boldsymbol{\tau}, \boldsymbol{\beta}) p(\boldsymbol{\beta}|\boldsymbol{\tau}, \boldsymbol{y})p(\boldsymbol{\tau}|\boldsymbol{y})d\bfs{\beta}d\bfs{\tau}
\end{equation}$$
- The integral on $\bfs{\beta}$ in (\ref{pdic_int}) is evaluated by Monte Carlo since $p(\boldsymbol{\beta}|\boldsymbol{\tau}, \boldsymbol{y})$ is roughly Gaussian. 
- We approximate the integral of $p(\boldsymbol{\tau}|\boldsymbol{y})$ by a trapezoidal rule. 
- The $ D(\hat{\bfs{\beta}}, \hat{\bfs{\tau}})$ is just a plug-in estimate using the posterior mean estimate of $\bfs{\beta}$ and the posterior mode of $\bfs{\tau|\bfs{y}}$.
...
- The first application is a density estimation presented in the \cite{hothorn2018most} using CTM.
- The second is a regression problem with only one explanatory variable \cite{hothorn2018most}.
- The last one is presented by () and involves more explanatory variables and random effects in a BCTM. 
...
- The \textit{mlt} package implements the CTM model using the Bernstein basis of dimension eight instead of the B-spline basis.
...
.tg .tg-eqm3{border-color:fff;font-size:20px;text-align:left;vertical-align:top}
</style>
<table class="tg">
...
```{r, echo=FALSE, out.width = "80%", dpi = 300 , fig.cap="Caption", fig.retina = 2,fig.align='center'}
...
- We considered a new data set which the observations are measures of age ($x$) and head circumference ($y$) of 7040 young Dutch boys.
...
```
---
- used the \textit{mlt} package to fit a conditional transformation model for head circumference with explanatory variable $x = age^{1/3}$. 
...
```{r, echo=FALSE, out.width = "100%", dpi = 300 , fig.cap="Caption", fig.retina = 2,fig.align='center'}
...
- We also fitted the following model
...
- carlan2023bayesian randomly selected 200 participants from the study and their respective cholesterol levels, gender and age measured at the beginning of the study and every two years for ten years.
